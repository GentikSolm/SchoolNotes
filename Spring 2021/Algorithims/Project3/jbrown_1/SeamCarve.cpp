...
int main(int argc, char** argv){
  ...
  int** imageMatrixFlip = new int*[cols];
  for(int i = 0; i< cols; i++){
    imageMatrixFlip[i] = new int [rows];
  }
  ...
  for(int i = 0; i < rows; i++){
    for(int j = 0; j < cols; j++){
      imageMatrixFlip[j][i] = imageMatrix[i][j];
    }
  }
  for(int i = 0; i < horiz; i++){
    int** energyMat = genEnergyMatrix(imageMatrixFlip, cols, rows);
    int** energySumMat = vertE(energyMat, cols, rows);
    vertSeam(imageMatrixFlip, energySumMat, cols, rows);
    delEnergyMatrix(energyMat, energySumMat, cols);
    rows--;
  }
  for(int i = 0; i < rows; i++){
    for(int j = 0; j < cols; j++){
      imageMatrix[i][j] = imageMatrixFlip[j][i];
    }
  }
  ...
  for(int i=0; i<cols; i++){
    delete[] imageMatrixFlip[i];
  }
  delete[]imageMatrixFlip;
  delete[]imageMatrix;
  return 0;
}
